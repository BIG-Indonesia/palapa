// 18/12/2019 ++Pengembangan Fungsi Upload Palapa++ By Yudha SN
...
nodeManager.controller('LayersCtrl', function($rootScope, $scope, CONFIG, LAYER, $window, $http, $base64, $upload, $timeout, $state, $stateParams, olData, USER_ROLES) {
    ...
    $http.get(CONFIG.api_url + 'meta/list', { cache: false }).success(function(data) {
        $scope.listmetalinks = data;
    });

    $http.get(CONFIG.api_url + 'metakugi/list', { cache: false }).success(function(data) {
        $scope.listmetakugilinks = data;
    });

    $scope.isdownloadable = function() {
        for (var i = 0; i < $scope.listmetalinks.length; i++) {
            console.log($scope.listmetalinks[i])
        }
        for (var o = 0; o < $scope.listmetakugilinks.length; o++) {
            console.log($scope.listmetakugilinks[i])
        }
    }
    ...
    //yudha pengembangan upload palapa
    $scope.progress_shp = 0;
    $scope.progress_shx = 0;
    $scope.progress_dbf = 0;
    $scope.progress_prj = 0;
    $scope.FormatFile=0;
    $scope.ujicoba="";
    //End
    $scope.FileSelectShp = function($files) {
        console.log('INIT');
        console.log($files);
        //$files: an array of files selected, each file has name, size, and type.
        for (var i = 0; i < $files.length; i++) {
            ...
            (function(index) {
                ...
                console.log(params)
                $scope.upload[index] = $upload.upload({
                    url: CONFIG.api_url + 'uploadshp', // webapi url
                    method: "POST",
                    // data: { fileUploadObj: $scope.fileUploadObj },
                    file: $file,
                    params: params
                    ...
                    // get upload percentage
                    console.log('percent: ' + parseInt(100.0 * evt.loaded / evt.total));
                    $scope.progress_shp = parseInt(100.0 * evt.loaded / evt.total);
                }).success(function(data, status, headers, config) {
                    $scope.response = data;
                    //Comment Sementara
                    //angular.element(document.getElementById('eWNext'))[0].disabled = false;
                    //Tambahan Request Pak Rahman
                    $scope.FormatFile = 1;
                    $scope.ujicoba=$scope.response.RTN;
                    //End
                    bootbox.alert($scope.response.MSG)
                    // file is uploaded successfully
    ...
    $scope.FileSelectShx = function($files) {
        console.log('INIT');
        console.log($files);
        //$files: an array of files selected, each file has name, size, and type.
        for (var i = 0; i < $files.length; i++) {
            ...
            (function(index) {
                ...
                console.log(params)
                $scope.upload[index] = $upload.upload({
                    url: CONFIG.api_url + 'uploadshx', // webapi url
                    method: "POST",
                    // data: { fileUploadObj: $scope.fileUploadObj },
                    file: $file,
                    params: params
                    ...
                    // get upload percentage
                    console.log('percent: ' + parseInt(100.0 * evt.loaded / evt.total));
                    $scope.progress_shx = parseInt(100.0 * evt.loaded / evt.total);
                }).success(function(data, status, headers, config) {
                    $scope.response = data;
                    //angular.element(document.getElementById('eWNext'))[0].disabled = false;
                    //Tambahan Request Pak Rahman
    ...
    $scope.FileSelectDbf = function($files) {
        console.log('INIT');
        console.log($files);
        //$files: an array of files selected, each file has name, size, and type.
        for (var i = 0; i < $files.length; i++) {
            ...
            (function(index) {
                ...
                console.log(params)
                $scope.upload[index] = $upload.upload({
                    url: CONFIG.api_url + 'uploaddbf', // webapi url
                    method: "POST",
                    // data: { fileUploadObj: $scope.fileUploadObj },
                    file: $file,
                    params: params
                    ...
                    // get upload percentage
                    console.log('percent: ' + parseInt(100.0 * evt.loaded / evt.total));
                    $scope.progress_dbf = parseInt(100.0 * evt.loaded / evt.total);
                }).success(function(data, status, headers, config) {
                    $scope.response = data;
                    //angular.element(document.getElementById('eWNext'))[0].disabled = false;
                    //Tambahan Request Pak Rahman
    ...
    $scope.FileSelectPrj = function($files) {
        console.log('INIT');
        console.log($files);
        //$files: an array of files selected, each file has name, size, and type.
        for (var i = 0; i < $files.length; i++) {
            ...
            (function(index) {
                ...
                console.log(params)
                $scope.upload[index] = $upload.upload({
                    url: CONFIG.api_url + 'uploadprj', // webapi url
                    method: "POST",
                    // data: { fileUploadObj: $scope.fileUploadObj },
                    file: $file,
                    params: params
                    ...
                    // get upload percentage
                    console.log('percent: ' + parseInt(100.0 * evt.loaded / evt.total));
                    $scope.progress_prj = parseInt(100.0 * evt.loaded / evt.total);
                }).success(function(data, status, headers, config) {
                    $scope.response = data;
                    angular.element(document.getElementById('eWNext'))[0].disabled = false;
                    //Tambahan Request Pak Rahman
    ...
    $scope.MetaUpArcgis = function(id, title, abstract, akses, keyword) {
        console.log($scope.metaFile);
        console.log($scope.docFile);
        console.log($scope.curwrk);
        console.log($scope.custoinfo);
        ...
        parameters = {};
        //parameters.ID = id;
		parameters.ID = encodeURIComponent($scope.model.layer.layer_id);
		parameters.TITLE = encodeURIComponent($scope.model.layer.layer_title);
        parameters.KEYWORD = encodeURIComponent(keyword.keyword);
        /*
		try {
            if (typeof $scope.model.layer.layer_name == 'undefined') {
		...
		*/
        ...
        if ($scope.minimalmeta == true) {
            $http.post(CONFIG.api_url + 'minmetadataarc', data).success(function(data, status) {
                pesan = data;
                console.log(pesan);
                bootbox.alert(pesan.MSG);
            });
    ...
    $scope.advGSLayer = function() {
        var params = {
            ...
            nativename: $scope.model.layer.layer_nativename,
            tipe: $scope.model.layer.layer_type,
            downloadable: $scope.model.layer.downloadable
        };
        console.log(params);
        var data = $.param({
            json: JSON.stringify({
    ...
    AdvLayerDialogModel.prototype.open = function(layer) {
        this.layer = layer;
        console.log(layer);
        this.layer.downloadable = 'N';
        this.layer.akses = '';
        for (var i = 0; i < $scope.listmetalinks.length; i++) {
            console.log($scope.listmetalinks[i])
            if ($scope.listmetalinks[i].akses == 'PUBLIC') {
                if ($scope.listmetalinks[i].workspace + ':' + $scope.listmetalinks[i].identifier == this.layer.layer_nativename) {
                    this.layer.akses = $scope.listmetalinks[i].akses;
                    if ($scope.listmetalinks[i].donwloadable) {
                        this.layer.downloadable = $scope.listmetalinks[i].donwloadable;
                        this.layer.akses = $scope.listmetalinks[i].akses;
                    } else {
                        this.layer.downloadable = 'N';
                        this.layer.akses = $scope.listmetalinks[i].akses;
        ...
        for (var o = 0; o < $scope.listmetakugilinks.length; o++) {
            console.log($scope.listmetakugilinks[i])
            if ($scope.listmetalinks[i].akses == 'PUBLIC') {
                if ($scope.listmetalinks[i].workspace + ':' + $scope.listmetakugilinks[i].identifier == this.layer.layer_nativename) {
                    this.layer.akses = $scope.listmetalinks[i].akses;
                    if ($scope.listmetalinks[i].donwloadable) {
                        this.layer.downloadable = $scope.listmetakugilinks[i].donwloadable;
                        this.layer.akses = $scope.listmetalinks[i].akses;
                    } else {
                        this.layer.downloadable = 'N';
                        this.layer.akses = $scope.listmetalinks[i].akses;
